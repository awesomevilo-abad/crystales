...
class GenericMethod{
    ...
    public static function getBalanceAmountOfDocumentPO($payment_type,$company_id,$supplier_id,$po_no){
        $transactions = DB::table('transactions')
        ->leftJoin('p_o_batches','transactions.tag_id','=','p_o_batches.tag_id')
        ->where('payment_type',$payment_type)
        ->where('company_id',$company_id)
        ->where('supplier_id',$supplier_id)
        ->where('po_no',$po_no)
        ->orderBy('balance_document_po_amount','desc')
        ->take(1)
        ->get('balance_document_po_amount');
        return $transactions;
    }
    ...
    public static function validateIfPartialPONoExistInPRM($payment_type,$company_id,$supplier_id,$po_no,$document_amount){
        ...
        ->where('supplier_id',$supplier_id)
        ->where('po_no',$po_no);
        ->where('balance_document_po_amount','>=')
        return $transactions->count();
    }

...
class TransactionValidationMethod
 {
    publicc staticfunction validateExistingPOInCompany( $payment_type, $company_id,$po_no){
    ...
    publicc staticfunction validateExistingPOInSupplier( $payment_type, $supplier_id,$po_no){
        $transaction = DB::table('transactions')
        ->leftJoin('p_o_batches','transactions.tag_id','=','p_o_batches.tag_id')
        ->where('payment_type', $payment_type)
    ...
    publicc staticfunction validateExistingDocNoAndPONo( $payment_type, $document_no,$po_no){
        $transaction = DB::table('transactions')
        ->leftJoin('p_o_batches','transactions.tag_id','=','p_o_batches.tag_id')
        ->where('payment_type', $payment_type)
        ->where('document_no', $document_no)

...
class GenericMethod{
    ...
    public static function getUserDetailsById($id){
        ...
        foreach($final_document_details as $final_specific_document_details){
            ...
            $document_ids[]= $final_specific_document_details->masterlist_document_id;
        }

        $get_document_id_with_no_categories = DB::table('user_document_category')->where('category_id',0)
        ->get();

        // return $get_document_id_with_no_categories;
    ...
    public static function getCategoriesByUserAndDocID($user_id,$document_id){
    ...
    public static function getCategoriesFromRequest($request_document_types,$document_id){
        $new_request = collect();
        foreach($request_document_types as $specific_document_types){
            if($specific_document_types['document_id'] == $document_id){
             $categories->push([
                 "id" => $specific_document_types['document_id'],
                "categories" =>  $specific_document_types['categories']
             ]);
            }
        }

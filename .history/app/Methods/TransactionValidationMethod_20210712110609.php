        ...
        if (GenericMethod::validateIfDocumentNoExist($fields['document_no']) > 0){
            ...
            if($fields['payment_type'] == 'FULL'){
                ...
                if(!empty($po) || isset($po)){
                    ...
                    return "WALA laman and PO";
                    // if(!empty($referrence_group) || isset($referrence_group)){

                    //     $ref_count = count($ref);
                    //     $ref_validation_count = count($ref);
                    ...
                    //         $ref_no =  $ref_validation_object->ref_no;

                    //         $ref_validation_amount =(float) str_replace(',', '', $ref_validation_object->ref_amount);
                    //         $ref_validation_qty =(float) str_replace(',', '', $ref_validation_object->ref_qty);
                    ...
                    //     $response = [
                    //         "code" => 404,
                    //         "message" => "PO number and Reference number are empty",
                    //         "data" => null,
                    //     ];
                  ...
                  // Compile existed PO Number
                    ...
                    if(isset($po_validation_total_amount)){
                        if($fields['document_amount'] < $po_validation_total_amount){
                            $response =  [
                                "code" => 400,
                            ...
                            $response =  [
                                "code" => 400,
                                "message" => "Total PO Amount is lower than the total PO Amount",
                                "data" => null,
                            ];
                        }
                    }else if(isset($ref_validation_total_amount)){
                        if($fields['document_amount'] != $ref_validation_total_amount){
                            $response =  [
                                "code" => 400,
                                "message" => "Document amount must be equal to total Ref Amount",
                                "data" => null,
                            ];
                        }
                    }else{
                        $response = 'Validated INSERT FORMULA HERE';
                    }

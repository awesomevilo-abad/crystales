            ...
            if($fields['payment_type'] == 'FULL'){
                ...
                if((!empty($po)) && ((empty($referrence_group) )) ){
                    ...
                    for($i=0;$i<$po_validation_count;$i++){
                        ...
                        $po_from_db_object = GenericMethod::getUsedPOFromDB($fields['payment_type'],$fields['company_id'],$fields['supplier_id'],14001,$fields['document_amount']) > 0){
                            if ( $po_from_db_object->count();
                            // $used_po_from_db[] =  $po_no;
                        }
                    ...
                    if($response == "Validated"){
                        ...
                        if(isset($used_tag_id)){
                            // $response= "Existing Transaction";
                            if($po_no_list == $used_po_from_db){
                                $response = $used_po_from_db;
                            }else{
                                $response = [
                                    "code" => 403,
                                    "message" => "PO No. already exist from other Document No",
                                        "data" => null,
                                ];
                            }
                        }else{
                            $response= "New Transaction Here";
                        }
